...
@section('content')
    <main>
        ...
        <!-- Combo Box -->
        <section>
            <form action="{{ route('getMhsLulus') }}" method="POST">
                @csrf
                    ...
                    <table class="min-w-full divide-y-2 divide-gray-200 bg-white text-sm">
                        <thead class="ltr:text-left rtl:text-right">
                            <tr>
                                ...
                                <th class="whitespace-nowrap px-4 py-2 font-semibold text-base text-gray-900">Jumlah Lulusan
                                </th>
                                <th class="whitespace-nowrap px-4 py-2 font-semibold text-base text-gray-900">Rata-Rata IPK
                                </th>
                                <th class="whitespace-nowrap px-4 py-2 font-semibold text-base text-gray-900">Rata-Rata Masa
                                    Studi
                                </th>
                            </tr>
                        </thead>
                                ...
                                @foreach ($data as $row)
                                    <tr class="odd:bg-white-bone">
                                        <td class="whitespace-nowrap px-4 py-2 font-medium text-gray-900">
                                            TS-{{ --$ts . ' (' . $row['tahun'] . ')' }}</td>
                                        <td class="whitespace-nowrap px-4 py-2 text-gray-700">{{ $row['jml_lulusan'] }}</td>
                                        <td class="whitespace-nowrap px-4 py-2 text-gray-700">{{ $row['rerata_ipk'] }}
                                        <td class="whitespace-nowrap px-4 py-2 text-gray-700">{{ $row['rerata_masa_studi'] }}
                                        </td>
                                    </tr>
                                @endforeach
                            @else
