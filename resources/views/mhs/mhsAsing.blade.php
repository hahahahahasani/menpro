    ...
    <main>
        ...
        <section>
            <div class="container mx-auto mt-28 text-center">
                <div class="title">
                    <h1 class="text-5xl font-extrabold text-slate-800">Data Mahasiswa Asing</h1>
                </div>
        ...
        <!-- Combo Box -->
        <section>
            <form action="{{ route('getMhsAsing') }}" method="POST">
                @csrf
                                ...
                                @foreach ($data as $row)
                                    <tr class="odd:bg-white-bone">
                                        <td class="whitespace-nowrap px-4 py-2 font-medium text-gray-900">
                                            TS-{{ --$ts . ' (' . $row['tahun'] . ')' }}</td>
                                        <td class="whitespace-nowrap px-4 py-2 text-gray-700">{{ $row['jml_mhs_asing'] }}</td>
                                        </td>
                                    </tr>
                                @endforeach
                            @else
